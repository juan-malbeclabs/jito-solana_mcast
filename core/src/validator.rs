    ...
    pub fn new_with_exit(
        ...
        *admin_rpc_service_post_init.write().unwrap() = Some(AdminRpcRequestMetadataPostInit {
        ...
        let multicast_shred_check_services = if !config.disable_multicast_shred_check
            ...
            // Development and Devnet do not advertise a multicast group;
            // multicast_root_receiver_address remains None for the lifetime
            // of the validator, so retransmit forwarding is silently a no-op.
            Vec::new()
        };
