...
use {
    crate::{
        ...
        multicast_shred_check_service::{
            MulticastShredCheckService, MULTICAST_ROOT_SHRED_ADDR_MAINNET,
            MULTICAST_ROOT_SHRED_ADDR_TESTNET, MULTICAST_SHRED_ADDR_MAINNET,
...
/// Spawns a polling thread that periodically rebuilds the kernel route table
/// and returns a closure that reports whether an IP is covered by a kernel
/// route with a non-zero prefix length. The retransmit stage uses it to gate
/// the multicast root receiver feature. Returns `(None, None)` on non-Linux or
/// when the route table cannot be read.
#[cfg(target_os = "linux")]
fn spawn_leader_route_check(
    exit: Arc<AtomicBool>,
) -> (Option<JoinHandle<()>>, Option<solana_turbine::LeaderRouteCheck>) {
    use {
        agave_xdp::route::Router,
        std::{
    ...
    let monitor_router = Arc::clone(&atomic_router);
    let handle = thread::Builder::new()
        ...
        .spawn(move || {
            ...
            while !exit.load(Ordering::Relaxed) {
                ...
                match Router::new() {
                    Ok(r) => monitor_router.store(Arc::new(r)),
                    Err(err) => warn!("multicast_root_receiver: route refresh failed: {err}"),
                }
            }
        })
